...
})(void 0, document, jQuery); // duration spinner
...
$('#people').multiSelect(); // time-picker
...
$(function () {
  ...
  $('input[placeholder], textarea[placeholder]').placeholder(); // display modal on top of modal
  ...
  $(document).on('click', '.change_status2', function () {
    $('.status2').toggle(); // $(this).prop("disabled", true);
    // $(this).html(
  ...
  $(".next").click(function () {
    ...
    next_fs = $(this).closest('fieldset').next(); //Add Class Active , Done

    $("#progressbar li").eq($("fieldset").index(next_fs)).addClass("active");
    $("#progressbar li").eq($("fieldset").index(current_fs)).addClass("done"); //show the next fieldset
  ...
  $(".previous").click(function () {
    current_fs = $(this).closest('fieldset');
    previous_fs = $(this).closest('fieldset').prev(); //Remove class active
  ...
  $(".selNumber").select2(); // Read selected option
  ...
  $('#noti_Button').click(function () {
    ...
    $('#notifications').fadeToggle('fast', 'linear'); //$('#noti_Counter').fadeOut('slow');     // HIDE THE COUNTER.
  ...
  function modalTemp(modalId, modalSize, modalTitle) {
    return ['<div class="modal" tabindex="-1" role="dialog" id="' + modalId + '">', '<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable ' + modalSize + '" role="document">', '<div class="modal-content">', '<div class="modal-header">', '<h5 class="modal-title">' + modalTitle + '</h5>', '<button type="button" class="close" data-dismiss="modal" aria-label="Close">', '<span aria-hidden="true">&times;</span>', '</button>', '</div>', '<div class="modal-body" id="modalBody"></div>', '</div>', '</div>', '</div>'].join('');
  }
...
$(document).on('click', '#changeToLight', function () {
  localStorage.clear();
  localStorage.setItem('theme', themeObj);
  var themeVal = localStorage.getItem('themeObj');
  $('html').addClass(themeVal.light);
  $('html').removeClass('dark');
});
$(document).on('click', '#changeToDark', function () {
  localStorage.clear();
  localStorage.setItem('theme', themeObj);
  var themeVal = localStorage.getItem('themeObj');
  $('html').addClass(themeVal.dark);
...
document.getElementById("arabicLang").addEventListener('click', function (e) {
  ...
  if (srcCalendarEl) {
    var srcCalendar = new FullCalendar.Calendar(srcCalendarEl, {
      ...
      eventLeave: function eventLeave(info) {
  ...
  $('html').attr('dir', 'ltr'); // $('.switchFormContainer')[0].style.display = 'none';

  $('.top-header')[0].style.background = '#fff url("../images/topleft-right.png") no-repeat right center';
  $('.jquery-timepicker-am-button').text('AM');
  $('.jquery-timepicker-pm-button').text('PM');
...
document.getElementById("englishLang").addEventListener('click', function (e) {
  ...
  $('html').attr('dir', 'rtl'); // $('.switchFormContainer')[0].style.display = 'flex';

  $('.top-header')[0].style.background = '#fff url("images/bg_art_topleft.png") no-repeat left center';
  $('.jquery-timepicker-am-button').text('ص');
  $('.jquery-timepicker-pm-button').text('م');
  $('#selector').calendarsPicker('destroy'); // disable datepicker and field 
